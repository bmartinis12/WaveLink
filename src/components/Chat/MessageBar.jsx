...
function MessageBar() {
  ...
  const sendMessage = async () => {
    try {
      ...
      dispatch({ type: reducerCase.ADD_MESSAGE, newMessage: { ...data.message }, fromSelf: true });
      setMessage('')
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div className="h-20 px-4 flex items-center gap-6 sm:gap-2 md:gap-6 relative bg-panel-header-background">
      {!showAudioRecorder && (
        <>
          <div className="flex gap-6">
            <BsEmojiSmile className="text-panel-header-icon cursor-pointer text-xl hidden sm:inline" title='Emoji' id='emoji-open' onClick={handleEmojiModal} />
            {showEmojiPicker && (
              <div className='absolute bottom-24 left-16 z-50' ref={emojiPickerRef}>
